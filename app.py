...
def load_model_and_vectorizer():
    ...
    # پیدا کردن ستون متن - تطبیق با نام‌های مختلف
    text_column = None
    possible_names = ['clean_text', 'text', 'tweet', 'content', 'review']
    ...
    if text_column is None:
        # اگر هیچکدام نبود، اولین ستون متنی را انتخاب کن
    ...
    label_column = None
    possible_labels = ['sentiment', 'label', 'class', 'target']
    ...
    if label_column is None:
        # اگر هیچکدام نبود، آخرین ستون را به عنوان برچسب در نظر بگیر
        label_column = df.columns[-1]
    
    X = df[text_column].values
    y = df[label_column].values
    
    # اگر برچسب‌ها رشته‌ای هستند، به عددی تبدیل کن
    if y.dtype == 'object':
        # فرض می‌کنیم مقادیر مثبت/منفی یا 0/1 هستند
        unique = np.unique(y)
        if len(unique) == 2:
            # اگر مقادیر مثبت/منفی هستند
            if 'Positive' in unique or 'positive' in unique:
                y = np.array([1 if str(val).lower() in ['positive', 'pos', '1'] else 0 for val in y])
            else:
                # تبدیل خودکار به 0 و 1
