...
def load_model_and_vectorizer():
    ...
    for col in df.columns:
        if df[col].dtype == 'object':
            # اولین ستون با متن را انتخاب کن
            if df[col].str.len().mean() > 10:  # احتمالاً متن است
                text_col = col
                break
    
    if text_col is None:
        text_col = df.columns[0]  # ستون اول
    
    # پیدا کردن ستون برچسب
    label_col = None
    for col in df.columns:
        if col != text_col and df[col].dtype in ['int64', 'float64', 'object']:
            label_col = col
            break
    
    if label_col is None:
        label_col = df.columns[-1]  # آخرین ستون
    
    st.sidebar.write(f"📌 Using **{text_col}** as text column")
    st.sidebar.write(f"📌 Using **{label_col}** as label column")
    ...
    # اگر برچسب‌ها رشته‌ای هستند
    if y_raw.dtype == 'object':
        unique_labels = np.unique(y_raw)
        st.sidebar.write(f"📊 Unique labels: {unique_labels.tolist()}")
        
        # اگر دو کلاس داریم (مثبت/منفی)
        if len(unique_labels) == 2:
            # ساخت دیکشنری برای تبدیل
            label_map = {}
            for i, label in enumerate(unique_labels):
                label_map[label] = i
            y = np.array([label_map[val] for val in y_raw])
        else:
            st.error(f"Expected 2 classes, but found {len(unique_labels)}")
            st.stop()
    else:
        # اگر از قبل عددی هستند
        y = y_raw.astype(int)
    
    # اطمینان از اینکه فقط 0 و 1 داریم
    if len(np.unique(y)) != 2:
        st.error(f"Labels must be binary (0/1), found: {np.unique(y)}")
        st.stop()
